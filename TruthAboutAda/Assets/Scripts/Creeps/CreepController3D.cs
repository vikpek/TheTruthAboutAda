...
public class CreepController3D : MonoBehaviour
{
		...
		void Awake ()
		{
				if (transform.name == Constants.CREEP_BLACK)
						creepBlack = true;
				else if (transform.name == Constants.CREEP_SILVER)
						creepSilver = true;
		...
		{
				if (obj.parent.GetComponent<RowSpawner> () != null)
						_link = obj.parent.GetComponent<RowSpawner> ();
				else
						findMovVerCon (obj.parent);
		}
		...
		void OnTriggerEnter (Collider col)
		{
			...
			// TODO difficulty is super smelly and redundant... because i'm lazy.
						//hard
						if (PlayerPrefs.GetFloat (Constants.PREF_DIFFICULTY) > 1) {
		...
		void kickCreepDown ()
		{
				...
				foreach (Transform element in el)
						addConstForces (element.gameObject);
				creepDeath ();
		}
...
//		Destroy( GetComponent<CreepController3D>() );
		...
		void addConstForces (GameObject element)
		{
				...
				if (temp == null)
						temp = element.AddComponent<ConstantForce> ();
				Rigidbody rig = element.GetComponent<Rigidbody> ();
				if (rig == null)
						rig = element.AddComponent<Rigidbody> ();
		...
		void rotateCylinder ()
		{
				if (cylinderTransform)
						cylinderTransform.rotation = CylinderUtility.Get.rotateCylinder (cylinderTransform.rotation.eulerAngles, Random.Range (0, 9));
				rotateEnd = true;
		}
				...
				if (!destroyed) {
						...
						foreach (Collider coll in colliders) {
								if (coll.transform.tag == Tags.CREEP) {
										...
										if (coll.name == Constants.CREEP_SILVER && t != this)
												t.explodeYeah ();
										else
												t.explodeMe ();
								}
